                                ...
                                <div class="card border-0 shadow-lg rounded-4" 
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            {{-- <div class="col-md-4">
                                                <div class="form-floating">
                                                    <input type="number" class="form-control rounded-3" id="gst" 
                                                        name="gst" placeholder="0.00"
                                                        style="background: #fff; color: #6267ae; border: 1px solid #f6b51d;" >
                                                    <label for="total" style="color: #6267ae;">GST(%)</label>
                                                </div>
                                            </div> --}}
...
$(document).ready(function () {
    ...
    // Event listener for discount input and type change
...
$('#savePet').on('click', function () {
    ...
    $.ajax({
        ...
        error: function (xhr) {
            let errors = xhr.responseJSON?.errors;
            let errorMessage = xhr.responseJSON?.message || 'An error occurred while adding the pet and owner.';
            ...
            if (errors) {
                ...
                Object.keys(errors).forEach(field => {
                    ...
                    let $errorElement = $field.next('.text-danger');
                    if ($errorElement.length === 0) {
                        $errorElement = $('<div class="text-danger text-sm mt-1"></div>');
...
$('#addPetModal').on('hidden.bs.modal', function () {
    $('#addPetForm')[0].reset();
    // Clear file inputs
    $('#addPetForm').find('input[type="file"]').val('');
    $('.text-danger').addClass('d-none').text('');
    $('.is-invalid').removeClass('is-invalid');
});

// Also reset form when modal is shown
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    //    const total = parseFloat($('#total').val()) || 0;  
    //   const gstpercent = parseFloat($('#gst').val()) || 0;  

    //    const gstAmount = (total * gstpercent) / 100; 
    //    const newTotal = total + gstAmount; 
                        ...
                        data.forEach(item => {
                            ...
                            li.addEventListener("click", function () {
                                if (item.type === 'profile') {
                                    ...
                                    fetch(`{{ url('admin/profiles') }}/${item.id}/tests`)
                                        .then(response => response.json())
                                        .then(profileTests => {
