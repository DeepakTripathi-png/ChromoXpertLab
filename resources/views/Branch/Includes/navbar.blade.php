...
<div class="navbar-custom">
    <div class="d-flex align-items-center">
        <a href="{{ url('/branch/dashboard') }}" class="ms-3">
    ...
    <ul class="list-unstyled topnav-menu float-end mb-0">
        {{-- Notifications --}}
        @if(Auth::guard('branch')->check() && str_contains(Auth::guard('branch')->user()->role->privileges ?? '', 'notifications_view'))
            <li class="dropdown notification-list topbar-dropdown">
                <a class="nav-link waves-effect waves-light" href="{{ url('branch/notifications') }}" aria-label="Notifications">
        ...
        <li class="dropdown notification-list topbar-dropdown">
            <a class="nav-link dropdown-toggle nav-user me-0" data-bs-toggle="dropdown" href="#" role="button" aria-haspopup="true" aria-expanded="false" aria-label="User profile">
                <img src="{{ 
                    !empty(Auth::guard('branch')->user()->branch_logo_path) && Storage::exists(Auth::guard('branch')->user()->branch_logo_path) ? url('/').Storage::url(Auth::guard('branch')->user()->branch_logo_path) :
            ...
            <div class="dropdown-menu dropdown-menu-end profile-dropdown">
                <div class="dropdown-header text-center">
                    <h6>
                        Welcome, {{ Auth::guard('branch')->user()->name ?? '' }}
                    </h6>
                ...
                <a href="{{ url('branch/logout') }}" class="dropdown-item"><i class="fe-log-out text-[#6267ae] me-2"></i> Logout</a>
            </div>
        </li>
    </ul>
</div>
        ...
        <div id="sidebar-menu">
            <ul id="side-menu">
                @if(Auth::guard('branch')->check())
                    ...
                    <li class="{{ Request::is('branch/report*') ? 'active' : '' }}">
                    ...
                    {{-- Test Reports --}}
                    @if(!empty($RolesPrivileges) && str_contains($RolesPrivileges->privileges, 'reports_view'))
                    <li class="{{ Request::is('branch/sample*') ? 'active' : '' }}">
                        <a href="{{ url('/branch/sample') }}">
                            <i class="mdi mdi-file-chart-outline"></i>
                            <span> Sample </span>
                        </a>
                    </li>
                    ...
                    <li class="{{ Request::is('branch/parent*') ? 'active' : '' }}">
                        <a href="{{ url('/branch/parent') }}">
                    ...
                    <li class="{{ Request::is('branch/pet*') ? 'active' : '' }}">
                        <a href="{{ url('/branch/pet') }}">
                    ...
                    <li class="{{ Request::is('branch/referee-doctors*') ? 'active' : '' }}">
                        <a href="{{ url('/branch/referee-doctors') }}">
                    ...
                    <li class="{{ Request::is('branch/system-user*', 'branch/roles-privileges*') ? 'active' : '' }}">
                        ...
                        aria-expanded="{{ Request::is('branch/system-user*', 'branch/roles-privileges*') ? 'true' : 'false' }}">
                            <i class="mdi mdi-account-group-outline"></i>
                            <span> System Users </span>
                            <span class="menu-arrow"></span>
                        </a>
                        <div class="collapse {{ Request::is('branch/system-user*', 'branch/roles-privileges*') ? 'show' : '' }}" id="system-user">
                            <ul class="nav-second-level">
                                @if(!empty($RolesPrivileges) && str_contains($RolesPrivileges->privileges, 'users_view'))
                                <li class="{{ Request::is('branch/system-user*') ? 'active' : '' }}">
                                    <a href="{{ url('branch/system-user') }}">
                                        <span> Users </span>
                                    </a>
                                </li>
                                @endif
                                @if(!empty($RolesPrivileges) && str_contains($RolesPrivileges->privileges, 'role_privileges_view'))
                                <li class="{{ Request::is('branch/roles-privileges*') ? 'active' : '' }}">
