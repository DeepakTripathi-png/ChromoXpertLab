        ...
        <div class="container-fluid">
            ...
            <div class="card border-0 shadow-lg rounded-4" 
                ...
                <div class="card-body p-4">
                    ...
                    <form action="{{ url('branch/appointments/store') }}" method="POST" enctype="multipart/form-data" id="registrationForm">
                        @csrf
                        <div class="row g-4">
                            ...
                            <input type="hidden" id="lab_id" name="lab_id" value="{{ $branchLogin->id ?? null }}">
                            ...
                            <div class="col-md-6">
                                <div class="form-floating">
                                    <input type="tel" class="form-control rounded-3" id="phone" 
                                        ...
                                        pattern="(\+91)?[0-9]{10}" title="Phone number must be 10 digits, with or without +91"
                                    ...
                                    <div class="card-body p-3">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            {{-- <div class="col-md-4">
                                                <div class="input-group">
                                                    <div class="form-floating flex-grow-1">
                                                        <input type="number" step="0.01" min="0" class="form-control rounded-3" id="discount_input" 
                                                            placeholder="0.00"
                                                            style="background: #fff; color: #6267ae; border: 1px solid #f6b51d;">
                                                        <label for="discount_input" id="discount_label" style="color: #6267ae;">Discount (₹)</label>
                                                    </div>
                                                    <select class="form-select rounded-end" id="discount_type" 
                                                            style="background: #fff; color: #6267ae; border-left: none; border: 1px solid #f6b51d; max-width: 60px;">
                                            ...
                                            <div class="col-md-4">
                                                <div class="input-group">
                                                    <div class="form-floating flex-grow-1">
                                                        <input type="number" step="0.01" min="0" class="form-control rounded-3" id="discount_value" 
                                                            name="discount_value" placeholder="0.00" value="{{ old('discount', 0) }}"
                                                            style="background: #fff; color: #6267ae; border: 1px solid #f6b51d;">
                                                        <label for="discount_value" id="discount_label" style="color: #6267ae;">Discount (₹)</label>
                                                    </div>
                                                    <select class="form-select rounded-end" id="discount_type" name="discount_type" 
                                                            style="background: #fff; color: #6267ae; border: 1px solid #f6b51d;">
                                                        <option value="amount">Amount</option>
                                                        <option value="percentage">Percentage (%)</option>
                                                    </select>
                       ...
                       <div class="col-12">
                            <div class="form-floating">
                                <input type="tel" class="form-control rounded-3" id="doctor_mobile" name="mobile" 
                                    pattern="(\+91)?[0-9]{10}" 
                                    title="Phone number can optionally start with +91 followed by 10 digits"
                                    style="background: rgba(255,255,255,0.95); border: 1px solid #f6b51d;">
                                <label for="doctor_mobile" style="color: #6267ae;">Mobile* (+91 optional)</label>
                            </div>
                        </div>
...
<script>
// Age Calculation Functions - Session Safe Version
function initializeAgeHandling(dobSelector, ageSelector) {
    const $dobInput = $(dobSelector);
    const $ageInput = $(ageSelector);

    // Clean up existing event listeners first
    $dobInput.off('change.ageHandler');
    $ageInput.off('blur.ageHandler click.ageHandler');

    // Handle DOB change
    $dobInput.on('change.ageHandler', function() {
    ...
    // Handle Age input
    $ageInput.on('blur.ageHandler', function() {
        if (!$(this).prop('readonly')) {
            const ageStr = $(this).val().trim();
            if (ageStr === '') {
                $dobInput.val('');
                return;
            }
    ...
    $ageInput.on('click.ageHandler', function() {
        if ($(this).prop('readonly')) {
...
function resetAgeInput($ageInput) {
...
function updateTotal() {
    ...
    let discountType = $('#discount_type').val() || 'amount';
    let inputVal = parseFloat($('#discount_input').val()) || 0;
    let discount;
    ...
    if (discount > subtotal) {
        if (discountType === 'amount') {
            $('#discount_input').val(subtotal.toFixed(2));
            discount = subtotal;
        } else {
...
$(document).ready(function () {
    ...
    $('#discount_input').off('input.totalUpdate');
    $('#paid_amount').off('input.paidAmountCheck');
    $('#registrationForm').off('submit.formSubmit');
    $('#pet_species').off('change.breedUpdate');
    $('#pet_weight').off('input.weightValidation');
    $('#saveRefereeDoctor').off('click.refereeSave');
    $('#savePet').off('click.petSave');
    $('#pet_id').off('change.petSelect');
    $('#phone').off('input.phoneCheck');
    $('#discount_type').off('change.discountType');

    // Event listener for discount input
    $('#discount_input').on('input.totalUpdate', function () {
        updateTotal();
    });

    // Event listener for discount type change
    $('#discount_type').on('change.discountType', function () {
        const type = $(this).val();
        const label = $('#discount_label');
        if (type === 'amount') {
            label.text('Discount (₹)');
        } else {
            label.text('Discount (%)');
    ...
    $('#paid_amount').on('input.paidAmountCheck', function () {
    ...
    // Form validation - Exclude hidden lab_id from client-side required checks
    $('#registrationForm').on('submit.formSubmit', function (e) {
    ...
    $('#pet_species').on('change.breedUpdate', function() {
    ...
    $('#saveRefereeDoctor').on('click.refereeSave', function () {
        let $btn = $(this);
        ...
        $.ajax({
            url: '{{ route("branch.refereedoctor.store-ajax") }}',
            method: 'POST',
            data: $form.serialize(),
            headers: {
    ...
    $('#savePet').on('click.petSave', function () {
        let $btn = $(this);
        ...
        $.ajax({
            url: '{{ route("branch.pet-and-parent.store") }}',
            method: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $('#pet_id').on('change.petSelect', function () {
        const petId = $(this).val();
        if (petId === 'new') {
            $('#addPetModal').modal('show');
        } else if (petId) {
            $.ajax({
                url: '{{ route("branch.get.pet.details", ":pet_id") }}'.replace(':pet_id', petId),
                method: 'GET',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
    ...
    $('#phone').on('input.phoneCheck', function () {
        ...
        phoneCheckTimeout = setTimeout(() => {
            ...
            $.ajax({
                url: '{{ route("branch.get.owner.pets.by.phone", ":phone") }}'.replace(':phone', phone),
                method: 'GET',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById('discount_type').addEventListener('change', function() {
        ...
        updateTotal(); // Trigger recalculation
    });
    ...
    searchInput.addEventListener("keyup", function () {
        ...
        searchTimeout = setTimeout(() => {
            ...
            fetch("{{ route('branch.tests.search') }}?q=" + encodeURIComponent(query))
                ...
                .then(data => {
                    ...
                    if (data.length > 0) {
                        data.forEach(item => {
                            ...
                            li.dataset.price = item.total_price;
                            li.dataset.type = item.type;

                            let displayText = `${item.name}`;
                            if (item.type === 'test') {
                                displayText += ` (${item.test_code ?? ''}) - ₹${item.total_price}`;
                            } else if (item.type === 'profile') {
                                displayText += ` (${item.test_code ?? ''}) - ₹${item.total_price} (${item.tests_count} tests)`;
                            }
                            ...
                            li.addEventListener("click", function () {
                                if (item.type === 'profile') {
                                    // Fetch and add tests from profile
                                    fetch(`{{ url('branch/profiles') }}/${item.id}/tests`)
                                        .then(response => response.json())
                                        .then(profileTests => {
                                            // Check for duplicates
                                            const alreadyAddedTests = profileTests.filter(t => selectedTests.includes(t.id));
                                            if (alreadyAddedTests.length > 0) {
                                                toastr.warning(`Some tests from this profile are already added: ${alreadyAddedTests.map(t => t.name).join(', ')}`);
                                                return;
                                            }

                                            // Create single profile card
                                            let card = document.createElement("div");
                                            card.classList.add("col");
                                            let testHiddens = '';
                                            profileTests.forEach(test => {
                                                testHiddens += `<input type="hidden" name="tests[]" value="${test.id}" data-price="0">`;
                                                selectedTests.push(test.id);
                                            });
                                            const testNames = profileTests.map(t => t.name).join(', ');
                                            card.innerHTML = `
                                                <div class="card border-0 shadow rounded-3 h-100">
                                                    <div class="card-body">
                                                        <h6 class="card-title mb-2">${item.name} <span class="badge bg-primary ms-2">${profileTests.length} tests</span></h6>
                                                        <p class="text-muted small mb-1">Code: ${item.test_code ?? ''}</p>
                                                        <p class="text-muted small mb-1">Tests: ${testNames}</p>
                                                        <p class="fw-bold mb-0">₹${item.total_price} (Profile Price)</p>
                                                        ${testHiddens}
                                                        <input type="hidden" name="profiles[]" value="${item.id}" data-price="${item.total_price}">
                                                        <button class="btn btn-sm btn-danger mt-2 remove-test-btn float-end" title="Remove Profile">X</button>
                                                    </div>
                                                </div>
                                            `;

                                            card.querySelector(".remove-test-btn").addEventListener("click", function () {
                                                // Remove all associated tests from selected
                                                profileTests.forEach(test => {
                                            ...
                                            searchInput.value = "";
                                            suggestionsBox.style.display = "none";
                                            toastr.success(`${profileTests.length} tests added from profile: ${item.name} at ₹${item.total_price}`);
                                        })
                                        .catch(error => {
                                            console.error('Error fetching profile tests:', error);
                                            toastr.error('Failed to load profile tests.');
                                        });
                                } else {
                                    // Add single test
                                    addTestCard(item);
    ...
    // Update total function (updated to handle discount type)
    function updateTotal() {
        let subtotal = 0;
        
        // Sum from all inputs with data-price (tests and profiles)
        ...
        if (discountType === 'percentage') {
            ...
            if (discount > subtotal) {
                ...
                toastr.warning('Amount discount cannot exceed subtotal.');
            }
        }
